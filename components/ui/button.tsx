...
const buttonVariants = cva(
  "btn group/button rounded-md border border-transparent bg-clip-padding text-xs tracking-widest whitespace-nowrap uppercase transition-all outline-none focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/30 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-error aria-invalid:ring-2 aria-invalid:ring-error/20 dark:aria-invalid:border-error/50 dark:aria-invalid:ring-error/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      ...
      size: {
        default:
          "px-6 has-data-[icon=inline-end]:pr-4 has-data-[icon=inline-start]:pl-4",
        xs: "btn-xs h-7 gap-1 px-3 has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2 [&_svg:not([class*='size-'])]:size-3",
        sm: "btn-sm h-9 gap-1 px-4 has-data-[icon=inline-end]:pr-3 has-data-[icon=inline-start]:pl-3",
        lg: "btn-lg h-11 px-8 has-data-[icon=inline-end]:pr-5 has-data-[icon=inline-start]:pl-5",
        icon: "btn-square",
        "icon-xs": "btn-square btn-xs size-7 [&_svg:not([class*='size-'])]:size-3",
        "icon-sm": "btn-square btn-sm size-9",
        "icon-lg": "btn-square btn-lg size-11",
      },
    },
    defaultVariants: {
      variant: "default",
